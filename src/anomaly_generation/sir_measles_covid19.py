...
def measles_sir_model(y: np.ndarray, t: float, params: Dict[str, Any]) -> list:
    ...
    #N_c = params['population']['children']
    N = N_a 
    
    beta_a = params['transmission_rates']['adults']
    #beta_c = params['transmission_rates']['children']
    gamma_1_a = params['progression_rates']['adults']['exposed_to_infected']
    #gamma_1_c = params['progression_rates']['children']['exposed_to_infected']
    gamma_2_a = params['progression_rates']['adults']['infected_stage1_to_stage2']
    #gamma_2_c = params['progression_rates']['children']['infected_stage1_to_stage2']
    gamma_3_a = params['progression_rates']['adults']['infected_stage2_to_stage3']
    #gamma_3_c = params['progression_rates']['children']['infected_stage2_to_stage3']
    recovery_rate_a = params['progression_rates']['adults']['infected_stage3_to_recovered']
    #recovery_rate_c = params['progression_rates']['children']['infected_stage3_to_recovered']
    ...
    # Children equations
    #dS_c = -beta_c * S_c * (I1_a + I2_a + I3_a + I1_c + I2_c + I3_c) / N
    #dE_c = beta_c * S_c * (I1_a + I2_a + I3_a + I1_c + I2_c + I3_c) / N - gamma_1_c * E_c
    #dI1_c = gamma_1_c * E_c - gamma_2_c * I1_c
    #dI2_c = gamma_2_c * I1_c - gamma_3_c * I2_c
    #dI3_c = gamma_3_c * I2_c - recovery_rate_c * I3_c
    #dR_c = recovery_rate_c * I3_c
...
class MeaslesSIRAnomalyGenerator(BaseAnomalyGenerator):
    ...
    def _generate_sir_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        initial_state = [
            ...
            #self.sir_params['initial_conditions']['children']['susceptible'],
            #self.sir_params['initial_conditions']['children']['exposed'],
            #self.sir_params['initial_conditions']['children']['infected_stage1'],
            #self.sir_params['initial_conditions']['children']['infected_stage2'],
            #self.sir_params['initial_conditions']['children']['infected_stage3'],
            #self.sir_params['initial_conditions']['children']['recovered']
        ]
    ...
    def _calculate_reported_symptoms(self, compartments: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        return {  
            'Fever_-_adult': symptom_probs['fever']['adults'] *0.8 *  (I1a),
            'Fever_-_child': symptom_probs['fever']['children'] *0.2* (I1a),
            'Cough_-_adult': symptom_probs['cough']['adults'] *0.8* (I2a),
            'Cough_-_child': symptom_probs['cough']['children'] *0.2* (I2a),
            'Respiratory_problems_-_adult': symptom_probs['respiratory_problems']['adults'] *0.8* (I3a), 
            'Respiratory_problems_-_children': symptom_probs['respiratory_problems']['children'] *0.2* (I3a)
        }
